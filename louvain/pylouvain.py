...
class PyLouvain:
    ...
    '''
    @classmethod
    def from_file(cls, path, source_data):
        f = open(path+source_data, 'r')
        lines = f.readlines()
        f.close()
        nodes = {}
        edges = []
        ...
        nodes_, edges_ = in_order(nodes, edges, path)
...
def in_order(nodes, edges, path):
        ...
        d = {}
        #store nodes info
        out_tmp = open(path+'nodes_tmp.txt', 'w')
        for n in nodes:
            out_tmp.write(str(n)+'\t'+str(i)+'\n') #turtle
            nodes_.append(i)
            d[n] = i
            i += 1
        out_tmp.close() #close file
        edges_ = []
        for e in edges:
